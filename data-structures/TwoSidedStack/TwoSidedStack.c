#include  <stdio.h>

#define MAX_SIZE 10

struct Stack
{
	int data[MAX_SIZE];
	int top1; /*FRONT*/
	int top2; /*REAR*/
};

void init_stack(struct Stack* p)
{
	p->top1 = -1;
	p->top2 = MAX_SIZE;
}

int isFull1(struct Stack* p)
{
	return (p->top1 >= p->top2-1);
}
int isEmpty1(struct Stack* p)
{
	return p->top1 == -1;
}

void push1(struct Stack* p, int val)
{
	if(!isFull1(p))	
		p->data[++p->top1] = val;
	else
		puts("Stack Overflow Occured");
}
int pop1(struct Stack* p)
{
	if(!isEmpty1(p))
		return p->data[p->top1--];
	else
	{
		puts("\nStack Underflow Occured. Ignore value obtained");
		return -1;
	}	
}

void display1(struct Stack* p)
{
	int i;	
	printf("{");
	for(i = p->top1; i > 0; --i)
	{
		printf("%d,",p->data[i]);
	}
	if(p->top1 >= 0)
		printf("%d",p->data[0]);
	printf("}");
}

int isEmpty2(struct Stack* p)
{
	return (p->top2 == MAX_SIZE);
}
int isFull2(struct Stack* p)
{
	return (p->top2 <= p->top1+1);
}
void push2(struct Stack* p, int val)
{
	if(!isFull2(p))	
		p->data[--p->top2] = val;
	else
		puts("Stack Overflow Occured");
}

int pop2(struct Stack* p)
{
	if(!isEmpty2(p))
		return p->data[p->top2++];
	else
	{
		puts("\nStack Underflow Occured. Ignore value obtained");
		return -1;
	}	
}
void display2(struct Stack* p)
{
	int i;	
	printf("{");
	for(i = p->top2; i < MAX_SIZE-1; ++i)
	{
		printf("%d,",p->data[i]);
	}
	if(p->top2 <= MAX_SIZE-1)
		printf("%d",p->data[MAX_SIZE-1]);
	printf("}");
}

int main()
{
	char ch;
	int val;

	struct Stack stack;

	init_stack(&stack);

	while(1)
	{
		puts("\nEnter 1 to PUSH Stack 1");
		puts("Enter 2 to PUSH Stack 2");
		puts("Enter 3 to POP Stack 1");
		puts("Enter 4 to POP Stack 2");
		puts("Enter 5 to DISPLAY Stack 1");
		puts("Enter 6 to DISPLAY Stack 2");
		puts("Enter 7 to Exit");

		scanf(" %c",&ch);
		switch(ch)
		{
			case '1':
				printf("\nAdd Data 1\t:");
				scanf("%d",&val);
				push1(&stack, val);
			break;
			case '2':
				printf("\nAdd Data 2\t:");
				scanf("%d",&val);
				push2(&stack, val);				
			break;
			case '3':
				val = pop1(&stack);
				printf("\nPopped out 1 : %d",val);
			break;
			case '4':
				val = pop2(&stack);
				printf("\nPopped out 2 : %d",val);
			break;
			case '5':
				printf("\nData in Stack 1 is\t:");	
				display1(&stack);
			break;
			case '6':
				printf("\nData in Stack 2 is\t:");	
				display2(&stack);				
			break;
			case '7':
				return 0;
			break;
		}
	}
	return 0;
}

/*
Output:

Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
1

Add Data 1	:3

Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
2

Add Data 2	:5

Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
1

Add Data 1	:6

Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
2

Add Data 2	:7

Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
5

Data in Stack 1 is	:{6,3}
Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
6

Data in Stack 2 is	:{7,5}
Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
3

Popped out 1 : 6
Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
4

Popped out 2 : 7
Enter 1 to PUSH Stack 1
Enter 2 to PUSH Stack 2
Enter 3 to POP Stack 1
Enter 4 to POP Stack 2
Enter 5 to DISPLAY Stack 1
Enter 6 to DISPLAY Stack 2
Enter 7 to Exit
7
*/

