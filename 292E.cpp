...
struct segtree
{
    ...
    // rupd = range update
    void rupd(const int32_t &v, const int32_t &tl, const int32_t &tr, const int32_t &l, const int32_t &r, int op, int x)
    {
        if (l > tr || r < tl)
            return;
        if (tl >= l && tr <= r)
        ...
        int32_t tm = (tl + tr) >> 1;
        rupd(v << 1, tl, tm, l, r, op, x);
        rupd(v << 1 | 1, tm + 1, tr, l, r, op, x + tm - tl + 1);
    }
...
public:
    ...
    void rupd(const int32_t &l, const int32_t &r, int op, int x)
    {
        rupd(1, 0, len - 1, l, r, op, x);
    }
};
